...
if __name__ == '__main__':
    ...
    # 全結合層
    img_6 = tf.reshape(img_base, shape=[-1, 60*30*3])

    seed_num = 100
    W_1 = tf.Variable(tf.truncated_normal([60*30*3, seed_num], mean=0.0, stddev=0.1, dtype=tf.float32))
    b_1 = tf.Variable(tf.constant(0.1, shape=[seed_num]))

    img_7 = tf.nn.sigmoid(tf.matmul(img_6, W_1) + b_1)

    seed_num_2 = 50
    W_2 = tf.Variable(tf.truncated_normal([seed_num, seed_num_2], mean=0.0, stddev=0.1, dtype=tf.float32))
    b_2 = tf.Variable(tf.constant(0.1, shape=[seed_num_2]))

    img_8 = tf.nn.sigmoid(tf.matmul(img_7, W_2) + b_2)


    # ソフトマックス関数
    W_0 = tf.Variable(tf.truncated_normal([seed_num_2, 3], mean=0.0, stddev=0.1, dtype=tf.float32))
    b_0 = tf.Variable(tf.constant(0.1, shape=[3]))

    p_base = tf.matmul(img_8, W_0) + b_0
    ...
    with tf.Session() as sess:
        ...
        for i in range(training_num):
            ...
            result_p, w0, w1, b0, b1, result_loss = sess.run([p, W_0, W_1, b_0, b_1, loss],
            ...
            if (i+1)%100 == 0:
                ...
                test_datas = data_set.get_test_dataset()
                result_p, result_p_base, w0, w1, b0, b1, img6, img7 = sess.run([p, p_base, W_0, W_1, b_0, b_1, img_6, img_7],
                                                                               feed_dict={img_base: test_datas})
